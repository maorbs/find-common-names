...
public class NamesPatternMatcher implements Callable<SearchResult> {
	...
	public NamesPatternMatcher(String text, Map<String, Pattern> regexPattenMap, long startLineOffset,
	...
	public SearchResult call() throws Exception {
		...
		for (int i = 0; i < linesText.length; i++) {
			...
			regexNamesMap.entrySet().parallelStream().forEach(entry -> {
				...
				while (matcher.find()) {
					sr.appendOrCreateResult(entry.getKey(),
							new ResultOffset(startLineOffset + index + 1, matcher.start()));
				}
			});
		}
		return sr;
